...
import { GeorefAddress } from '@/types/georef';
...
interface AddressResultsListProps {
  ...
  onAddressSelect: (address: GeorefAddress) => void;
  loading?: boolean;
  error?: string | null;
  emptyMessage?: string;
  lastQuery?: string;
...
export const AddressResultsList: React.FC<AddressResultsListProps> = ({
  ...
  // Function to transform AddressResult to GeorefAddress
  const transformToGeorefAddress = (address: AddressResult): GeorefAddress => {
    return {
      id: address.calle.id,
      nomenclatura: address.nomenclatura,
      calle: {
        id: address.calle.id,
        nombre: address.calle.nombre,
        categoria: address.calle.categoria,
      },
      altura: {
        numero: address.altura.valor, // Transform 'valor' to 'numero'
        unidad: address.altura.unidad,
      },
      localidad_censal: {
        id: address.localidad_censal.id,
        nombre: address.localidad_censal.nombre,
      },
      departamento: {
        id: address.departamento.id,
        nombre: address.departamento.nombre,
      },
      provincia: {
      ...
      ubicacion: address.ubicacion ? {
        lat: address.ubicacion.lat,
        lon: address.ubicacion.lon,
      } : undefined,
    };
  };
        ...
        <div className="p-2">
          ...
          {addresses.map((address, index) => (
            <button
              ...
              onClick={() => onAddressSelect(transformToGeorefAddress(address))}
