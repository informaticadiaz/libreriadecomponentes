...
export const useTrainerState = (props: TablesTrainerProps): UseTrainerStateReturn => {
  ...
  const nextQuestion = useCallback(() => {
    ...
    if (hasMoreQuestions) {
      ...
      // Quiz terminado, actualizar progreso
      const updatedProgress = updateTableProgress(
        state.practiceSettings.selectedTables[0], // Simplificado para el ejemplo
        state.questions,
        progress
      );
  ...
  }, [state.isAnswered, state.questionIndex, state.questions, state.practiceSettings.selectedTables, progress, onProgressChange]);
  ...
  const canSubmit = state.currentQuestion && !state.isAnswered;
